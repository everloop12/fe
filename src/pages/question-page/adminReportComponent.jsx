...
const AdminReportComponent = ({ questionId, currentUser }) => {
    ...
    const getQuestionReports = async () => {
        setLoading(true)
        setReports([])
        const reportsR = await axiosInstance.get(`/reports/${questionId}`, { headers: { Authorization: `Bearer ${currentUser?.token}` } })
                            ...
                            <div className='tw-flex tw-justify-between tw-w-full'>
                                <Button onClick={() => {navigate(`/user/${report?.user.id}`) }}>
                                    <h3 className='tw-ml-4'>
                                        user: {report?.user?.displayName || report?.user?.email || "no username"}
                                    </h3>
                                </Button>
                                <div>
